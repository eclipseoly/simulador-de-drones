...
# Biblioteca Pillow: Essencial para carregar, redimensionar e exibir arquivos de imagem (como os ícones dos drones)
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.colisoesAtuais = 0
        self.concluidosAtuais = 0
        self.animando = False
        ...
        self.label_titulo = ctk.CTkLabel(
            self.menu_frame, text="Configurações", font=("Arial", 20, "bold"))
        # pady=20 dá um espaço em cima e embaixo do texto
        ...
        self.entrada_qtd = ctk.CTkEntry(
            self.menu_frame, placeholder_text="Qtd de Drones")
        # fill="x" faz a caixinha esticar horizontalmente para preencher o frame
        self.entrada_qtd.pack(pady=10, padx=20, fill="x")

        self.velocidade_qtd = ctk.CTkEntry(
            self.menu_frame, placeholder_text="Velocidade dos drones")
        ...
        self.botao_iniciar = ctk.CTkButton(
            self.menu_frame, text="Iniciar Simulação", command=self.iniciar)
        ...
        self.label_log = ctk.CTkLabel(
            self.menu_frame, text="Terminal de Eventos", font=("Arial", 14, "bold"))
        ...
        self.caixa_log = ctk.CTkTextbox(
            self.menu_frame, state="disabled", fg_color="#1e1e1e", text_color="#00ff00")
        ...
        self.mapa_canvas = ctk.CTkCanvas(
        ...
        self.mapa_canvas.pack(side="right", fill="both",
        ...
        img_drone = Image.open(caminho_drone).resize(
            (60, 60), Image.Resampling.LANCZOS)
        self.icone_drone = ImageTk.PhotoImage(img_drone)

        # 2. Carrega a Explosão
        caminho_explosao = os.path.join(
            diretorio_atual, "assets", "explosao.png")
        img_explosao = Image.open(caminho_explosao).resize(
        ...
        img_base = Image.open(caminho_base).resize(
        ...
        img_alvo = Image.open(caminho_alvo).resize(
            (50, 50), Image.Resampling.LANCZOS)
    ...
    def imprimir_log(self, mensagem):
        ...
        self.caixa_log.configure(
            state="normal")           # 1. Destrava a caixa de texto
        # 2. Insere a mensagem com um marcador '>'
        self.caixa_log.insert("end", f"> {mensagem}\n")
        # 3. Rola a barra de rolagem para a mensagem mais recente
        self.caixa_log.see("end")
        # 4. Trava a caixa novamente
        self.caixa_log.configure(state="disabled")

    def animar(self):
        # movimenta os drones na tela

        # verifica se a animacao comecou
        if not self.animando:
            return

        # atualiza a posicao de cada drone na gui e muda a imagem de colidir
        ...
        # verifica se mudou o numero de colisoes e concluidos
        if self.colisoesAtuais != simulation.quantColisoes:
            alvo = simulation.quantColisoes
            while self.colisoesAtuais < alvo:
                self.colisoesAtuais += 2
                self.imprimir_log(
                    f"Mais 2 drones colidiram. Total: {self.colisoesAtuais}. Tempo atual: {simulation.segs} (s)")

        if self.concluidosAtuais != simulation.quantConcluidos:
            alvo = simulation.quantConcluidos
            while self.concluidosAtuais < alvo:
                self.concluidosAtuais += 1
                self.imprimir_log(
                    f"{self.concluidosAtuais} chegaram ao destino. Tempo atual: {simulation.segs} (s)")

        # finaliza a animacao e espera 1 segundo para mostrar os graficos
        if (simulation.acabou == True):

            # CÁLCULO DAS MÉTRICAS EXTRAS

            total_drones = len(simulation.conjuntoDrones)

            # Cálculos de Percentual e Taxa (evita divisão por zero)
            taxa_colisao = (simulation.quantColisoes /
                            total_drones) * 100 if total_drones > 0 else 0
            perc_sucesso = (simulation.quantConcluidos /
                            total_drones) * 100 if total_drones > 0 else 0

            # O tempo médio é a soma dos tempos dividida pelos que chegaram
            tempo_medio = (simulation.somaTemposChegada /
                           simulation.quantConcluidos) if simulation.quantConcluidos > 0 else 0

            # A distância final é o último valor gravado no gráfico de distâncias
            dist_media_final = metrics.distancia[-1] if metrics.distancia else 0

            # IMPRIME O RELATÓRIO FINAL COMPLETO NO TERMINAL

            self.imprimir_log(" ")
            self.imprimir_log("RELATÓRIO DE MÉTRICAS: ")
            self.imprimir_log(f"> Drones no Mapa: {total_drones}")
            self.imprimir_log(
                f"> Não completaram A Missão(Colidiram): {simulation.quantColisoes}")
            self.imprimir_log(
                f"> Chegaram ao destino: {simulation.quantConcluidos}")
            self.imprimir_log("- - - - - - - - - - - - - - - - - - - - -")
            self.imprimir_log(f"> Taxa de Colisão: {taxa_colisao:.1f}%")
            self.imprimir_log(f"> Percentual de Sucesso: {perc_sucesso:.1f}%")
            self.imprimir_log(
                f"> Tempo Total da Simulação: {simulation.segs:.2f} s")
            self.imprimir_log(f"> Tempo Médio p/ Chegada: {tempo_medio:.2f} s")
            self.imprimir_log(
                f"> Distância Média Percorrida: {dist_media_final:.2f} px")
            self.imprimir_log(
                f"> Passos/Iterações Executadas: {simulation.passosGerais}")
    ...
    def resetar(self):
        ...
        # zera as variaveis usadas no log
        self.colisoesAtuais = 0
        self.concluidosAtuais = 0

        # cancela a animacao
        if self.animacaoId:
            self.after_cancel(self.animacaoId)
            self.animacaoId = None
    ...
    def iniciar(self):
        ...
        self.imprimir_log(
            "Todos os assets foram desenhados na tela com sucesso!")
        ...
        # Cria os drones no motor lógico
        simulation.criandoDrones(qtd_texto, largura, altura, velocidade)

        # --- NOVA VERIFICAÇÃO DE LIMITE ---
        # Conta quantos drones realmente foram gerados com sucesso
        qtd_real = len(simulation.conjuntoDrones)

        # Se a quantidade real for menor que a solicitada, avisa o usuário no terminal da GUI
        if qtd_real < qtd_texto:
            self.imprimir_log(
                f"[ALERTA] O mapa lotou! Limite máximo atingido.")
            self.imprimir_log(
                f"Iniciando simulação com apenas {qtd_real} drones.")
        else:
            self.imprimir_log(
                f"Iniciando simulação... Preparando {qtd_real} drones.")
        # ---------------------------------

        # Desenha os drones que foram gerados na tela
        ...
        # Passa a quantidade REAL (qtd_real) para a thread, e não a que o usuário digitou
        self.threadSimulacao = threading.Thread(
            target=simulation.locomocao, args=(simulation.conjuntoDrones, qtd_real))
