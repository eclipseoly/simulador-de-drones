...
class App(ctk.CTk):
    ...
    def animar(self):
        ...
        if self.colisoesAtuais != simulation.quantColisoes:
            ...
            while self.colisoesAtuais < alvo:
                ...
                self.imprimir_log(f"Mais 2 drones colidiram. Total: {self.colisoesAtuais}. Tempo atual: {simulation.segs} (s)")
        ...
        if self.concluidosAtuais != simulation.quantConcluidos:
            ...
            while self.concluidosAtuais < alvo:
                ...
                self.imprimir_log(f"{self.concluidosAtuais} chegaram ao destino. Tempo atual: {simulation.segs} (s)")
        ...
        if(simulation.acabou == True):
            ...
            taxa_colisao = (simulation.quantColisoes / total_drones) * 100 if total_drones > 0 else 0
            perc_sucesso = (simulation.quantConcluidos / total_drones) * 100 if total_drones > 0 else 0
            
            # O tempo médio é a soma dos tempos dividida pelos que chegaram
            tempo_medio = (simulation.somaTemposChegada / simulation.quantConcluidos) if simulation.quantConcluidos > 0 else 0
            ...
            self.imprimir_log(f"> Não completaram A Missão(Colidiram): {simulation.quantColisoes}")
            self.imprimir_log(f"> Chegaram ao destino: {simulation.quantConcluidos}")
            self.imprimir_log("- - - - - - - - - - - - - - - - - - - - -")
            self.imprimir_log(f"> Taxa de Colisão: {taxa_colisao:.1f}%")
            self.imprimir_log(f"> Percentual de Sucesso: {perc_sucesso:.1f}%")
            self.imprimir_log(f"> Tempo Total da Simulação: {simulation.segs:.2f} s")
            self.imprimir_log(f"> Tempo Médio p/ Chegada: {tempo_medio:.2f} s")
            self.imprimir_log(f"> Distância Média Percorrida: {dist_media_final:.2f} px")
            self.imprimir_log(f"> Passos/Iterações Executadas: {simulation.passosGerais}")
    ...
    def iniciar(self):
        ...
        if qtd_real < qtd_texto:
            self.imprimir_log(f"[ALERTA] O mapa lotou! Limite máximo atingido.")
            self.imprimir_log(f"Iniciando simulação com apenas {qtd_real} drones.")
        else:
            self.imprimir_log(f"Iniciando simulação... Preparando {qtd_real} drones.")
        # ---------------------------------
        ...
        self.threadSimulacao = threading.Thread(target=simulation.locomocao, args=(simulation.conjuntoDrones, qtd_real))
